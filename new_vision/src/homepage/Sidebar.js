...
const Sidebar = ({ isOpen, setCompanyId, setYear, setCompanyname }) => {
  ...
  useEffect(() => {
    // Fetches available years from the server when component mounts
  ...
  // Handles navigating to the comparison page with the selected parameters
  useEffect(() => {
    if (
      isComparing &&
      selectedCompany &&
  ...
  // Handles input changes, filters companies based on the search term
  ...
  // Finalizes the selection of the company
  ...
  // Toggles the comparison mode
  const toggleCompare = () => setIsComparing(!isComparing);

  // Renders the sidebar component with conditional display logic
  return (
