...
const MetricsCard = ({ currentFramework }) => {
  ...
  useEffect(() => {
    ...
    Object.keys(frameworkMetrics).forEach(key => {
      frameworkMetrics[key].forEach(indicator => {
        indicator.subMetrics.forEach(subMetric => {
          if (subMetric.weight === undefined) {
            subMetric.weight = 1;  // 确保每个 subMetric 都有 weight 默认值
          }
        });
      });
    });
  ...
  const toggleSubMetrics = (id) => {
    setMetrics(metrics => metrics.map(metric =>
      metric.id === id ? { ...metric, isOpen: !metric.isOpen } : metric
    ));
  };

  // 定义 updateWeight 函数，用于更新权重
  const updateWeight = (id, newWeight, isSubMetric = false, subMetricId = null) => {
        ...
        if (!isSubMetric) {
          ...
          const updatedSubMetrics = metric.subMetrics.map(subMetric => {
            if (subMetric.id === subMetricId) {
              return { ...subMetric, weight: isValid ? weight : subMetric.weight };
            }
            return subMetric;
          });
    ...
    if (foundInvalid && !showGlobalError) {
      setShowGlobalError(true);
      setTimeout(() => {
        setShowGlobalError(false);
      }, 1000); // Show global error message for 1 second
    }
  };
  ...
  return (
    <Card className="metrics-card">
          ...
          {metrics.map(metric => (
            <ListGroup.Item key={metric.id} className="metric-item">
              <div className="d-flex justify-content-between align-items-center">
                ...
                >
                  <span className="info-icon" onClick={() => fetchMetrcisContent(metric.id)}>!</span>
                </OverlayTrigger>
                <div>
                <Form.Control
                  type="number"
                ...
                />
                {errors[metric.id] && <Alert variant="danger">Invalid weight (0-10)</Alert>}
                  <Button
      ...
      {metric.subMetrics.map((subMetric) => (
        <ListGroup.Item key={subMetric.id} className="d-flex align-items-center justify-content-between pe-3">
          ...
          <Form.Check
            ...
            label={
              <div className="label-container">
                  <span className="label-title">{subMetric.title}</span>
                  <span className="label-score">{subMetric.score}</span>
                </div>
            }
          ...
          <div className="d-flex align-items-center">
            ...
            >
              ...
              <span className="info-icon me-3" onClick={() => fetchIndicatorContent(metric.id)}>!</span>
            </OverlayTrigger>
